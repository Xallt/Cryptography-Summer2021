...
public:
    ...
    pair<T, T> encrypt(T message, T pub) {
        //cout << "message: ";output_poly(cout, message);
        //cout << "pub: ";output_poly(cout, pub);
        ull b = rand() % (p - 1) + 1;
        //cout << "B: " << salt << '\n';
        //cout << "salt: ";output_poly(cout, pow(ops->create(pub), salt).get());
...
Poly<typename ModuloOps::Elem> input_poly(istream &in, const ModuloOps *ops) {
    Poly<typename ModuloOps::Elem> f;
    string fs;
    getline(cin, fs);
    stringstream fss(fs);
    ll x;
...
int main(int argc, char *argv[]) {
    ...
    ModuloOps G(p);
    string s;
    Poly<ModuloOps::Elem> f = input_poly(cin, &G);
    Poly<ModuloOps::Elem> g = input_poly(cin, &G);
    Poly<ModuloOps::Elem> k = input_poly(cin, &G);
    getline(cin, s);

    PolyOps<ModuloOps::Elem> polyops(&G, f);
    list<ull> ls;
    for (int i = 0; i < s.size(); ++i)
        ls.push_back(char_to_number(s[i]));
    BigInt bs(ls, 64);
    bs = convert(bs, p);

    ElGamalEncryptionSystem<PolyOps<ModuloOps::Elem>> enc(&polyops, p, g);
    while (!bs.d.empty()) {
        Poly<ModuloOps::Elem> x;
        for (int i = 0; !bs.d.empty() && i < f.size() - 1; ++i) {
            x.d.push_back(G.create(bs.d.front()));
            bs.d.pop_front();
        }
        auto[m, gb] = enc.encrypt(x, k);
        output_poly(cout, gb);
        output_poly(cout, m);
    }
}
